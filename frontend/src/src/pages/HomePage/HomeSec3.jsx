...
import { ShieldCheck, BarChart3, Database, Workflow, Clock, ArrowRight, Brain, Activity } from 'lucide-react';
  ...
  return (
    <div className="w-full bg-[#020617] border border-white/5 rounded-[2rem] p-8 md:p-12 overflow-hidden relative group">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6 relative z-10">
        <div>
          <h3 className="text-xl md:text-2xl font-black text-white mb-2 tracking-tight">Global Fitness Adoption, 2020–2026</h3>
          <p className="text-gray-500 text-sm font-medium">Verified autonomous training sessions successfully completed — the urgency for precision has never been greater.</p>
        </div>
        <div className="text-right">
          <div className="text-3xl md:text-4xl font-black text-[#10B981] tracking-tighter">94%</div>
          <div className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Protocol Success Rate</div>
        </div>
      </div>
      ...
      <div className="relative h-64 w-full mt-8">
        ...
        <AnimatePresence>
          {hoveredPoint !== null && (
            ...
            >
              <div className="text-[10px] font-black text-gray-400 uppercase mb-1">{years[hoveredPoint]} Data</div>
              <div className="text-lg font-black text-white flex items-center gap-2">
                {data[hoveredPoint]}% <span className="text-[10px] text-[#10B981] font-bold">SUCCESS</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12 pt-12 border-t border-white/5">
        <div>
          ...
          <div className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Global Reps in 2024</div>
          <p className="text-xs text-gray-500 mt-2">Near all-time high healthcare burden — IEA, 2024</p>
        </div>
        <div>
          <div className="text-2xl font-black text-[#10B981]">−65.4%</div>
          <div className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Injury reduction</div>
          <p className="text-xs text-gray-500 mt-2">With AI-verified joint alignment vs unguided</p>
        </div>
        <div>
          <div className="text-2xl font-black text-white">$3.5T</div>
          <div className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Fit-Tech projected by 2030</div>
          <p className="text-xs text-gray-500 mt-2">Mordor Intelligence, 2025 Market Report</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        <div className="mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 mb-8">
            <span className="text-xs font-bold text-[#22D3EE] uppercase tracking-[0.2em]">Platform Excellence · 2026</span>
          </div>
          ...
          <p className="text-gray-500 max-w-2xl text-lg md:text-xl font-medium leading-relaxed">
            Everything you need to train with professional confidence — nothing you don't.
          </p>
        </div>
        ...
        <div className="mb-40">
           <div className="text-center mb-16">
             <h3 className="text-4xl font-black text-white mb-4">Platform at a glance</h3>
             <p className="text-gray-500 font-medium">Real numbers from active autonomous sessions on GenFit AI</p>
           </div>
           <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { val: "500,000+", label: "Reps tracked", sub: "Tons exercise volume" },
                { val: "8,500+", label: "Active athletes", sub: "Verified weekly users" },
                { val: "12+", label: "Clinical exercises", sub: "Verified protocols" },
                { val: "99%", label: "Accuracy rate", sub: "Independently verified" }
              ].map((m, i) => (
                <div key={i} className="text-center border-l border-white/5 pl-8 py-4">
                   <div className="text-3xl md:text-4xl font-black text-white mb-2 tracking-tighter">{m.val}</div>
                   <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{m.label}</div>
                   <div className="text-[9px] font-bold text-gray-600 uppercase tracking-widest">{m.sub}</div>
                </div>
              ))}
           </div>
        </div>
        ...
        <div className="pt-32 border-t border-white/5">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
            <div className="max-w-2xl">
              <h3 className="text-3xl md:text-5xl lg:text-6xl font-black text-white mb-6 tracking-tighter leading-none">From sign-up to <br /> first trade.</h3>
              <p className="text-gray-500 text-lg font-medium leading-relaxed opacity-80">No complex onboarding. No hidden steps. Just a straightforward path to trading verified fitness sessions.</p>
            </div>
            <div className="hidden lg:block pb-4">
               <button className="px-8 py-4 rounded-full bg-white text-black font-black text-sm uppercase tracking-widest hover:bg-emerald-50 transition-all">Get Started Free</button>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-40">
            {[
              { step: "01", title: "Create an account", desc: "Sign up in under 2 minutes with GitHub or Google. Complete your health profile to unlock protocols." },
              { step: "02", title: "Browse Protocols", desc: "Filter listings by muscle group, certification standard, equipment, and location." },
              { step: "03", title: "Buy or List Credits", desc: "Access premium programs or list your own verified sessions. Direct P2P value for everyone." },
              { step: "04", title: "Track Portfolio", desc: "View transaction history, certificates, and health performance curves in one dashboard." }
            ].map((s, i) => (
              <motion.div
                key={i}
              ...
              >
                <div className="text-8xl font-black text-white/[0.02] absolute -top-4 -right-2 select-none z-0 group-hover:text-[#22D3EE]/5 transition-colors uppercase leading-none">{s.step}</div>
                <div className="relative z-10 flex-1 flex flex-col">
                  <div className="w-14 h-14 rounded-2xl bg-[#020617] border border-white/10 flex items-center justify-center mb-10">
                    <span className="text-sm font-black text-[#22D3EE]">{s.step}</span>
                  </div>
                  <h4 className="text-xl font-black text-white mb-6 uppercase tracking-tight">{s.title}</h4>
          ...
          <div className="p-12 md:p-20 rounded-[3rem] md:rounded-[4rem] bg-gradient-to-br from-[#10B981] to-[#3B82F6] flex flex-col items-center justify-center gap-8 relative overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.5)]">
            ...
            <p className="text-[#E2E8F0] text-sm font-black uppercase tracking-[0.2em] z-10">Join 12,000+ athletes leading the health revolution.</p>
          </div>
        </div>
